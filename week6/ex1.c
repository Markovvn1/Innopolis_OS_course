...
int FCFS_scheduler(int N, struct ProcessAttr* attrs, struct SchedulerAttr** sattr)
{
	...
	for (int i = 0; i < resN; i++)
	{
		...
		{
			if (used[j]) continue;
			if (min_j != -1 && min_t <= attrs[j].arrival_time) continue;
...
int main()
{
	printf("%sSimple simulator of FCFS algorithm%s\n", C_BOLD_BLUE, C_CLEAR);
	...
	int scN = FCFS_scheduler(N, proc, &sched);
	if (N < 1)
	{
